...
const EmployeeTile = ({ emp, onDelete, onEdit, onFlag, onExpand, userRole }) => {
  return (
    <div className="glass-panel rounded-xl p-6 hover:-translate-y-1 transition-all duration-300 group relative overflow-hidden">
      {/* Status Bar */}
      ...
      <div className="flex justify-between items-start mb-4">
        ...
        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          {userRole === 'admin' && (
            <>
              <button onClick={() => onFlag(emp)} className="p-1.5 hover:bg-orange-50 rounded-full text-orange-400">
                <Flag size={16} />
              </button>
      ...
      <p className="text-sm mb-4" style={{ color: 'var(--text-secondary)' }}>{emp.class} • Age {emp.age}</p>
